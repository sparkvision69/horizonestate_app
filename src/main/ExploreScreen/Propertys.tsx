...
const App = ({filterdata, selectedFilter, navigation }:any) => {
  const [UpdatedData, setUpdatedData] = useState<any>([]);

  useEffect(() => {
    // Start by filtering based on the search query (filterdata)
    let FilteredData = PropertyData.filter((data) =>
      data.title.toLowerCase().trim().includes(filterdata.toLowerCase().trim())
    );

    // Apply the selected filter logic
    switch (selectedFilter) {
      case 'Low to High':
        FilteredData = [...FilteredData].sort((a, b) => a.price - b.price);  // Sort in ascending order
        break;
      case 'High to Low':
        FilteredData = [...FilteredData].sort((a, b) => b.price - a.price);  // Sort in descending order
        break;
      case 'Villa':
        FilteredData = FilteredData.filter((data) =>
          data.title.toLowerCase().includes('villa')
        );
        break;
      case 'Flat':
        FilteredData = FilteredData.filter((data) =>
          data.title.toLowerCase().includes('flat')
        );
        break;
      case 'Home':
        FilteredData = FilteredData.filter((data) =>
              ...
              <View style={styles.cardContent}>
                <View style={{
                  ...
                  <Text style={styles.title}>
                    {item.title.split(" ").slice(0, 1).join(" ")}{item.title.split(" ").length > 1 ? "..." : ""}
                  </Text>
